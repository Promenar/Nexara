...
源码始终位于当前 Git 工作树，依赖和缓存保持 Gradle/Android SDK 既有主机隔离。构建结果在 `native-ui/**/build/`，测试原始证据在 `artifacts/`，均不提交。现有 `secure_env/` 仅由既有安全签名路径消费，秘密不得回显或纳管。
