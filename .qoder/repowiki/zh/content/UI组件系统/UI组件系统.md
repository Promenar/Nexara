...
- 聊天相关组件：src/features/chat/components（消息气泡、输入、上下文菜单等）
...
## 结论
Nexara 的 UI 组件系统以“主题—颜色—动画—组件”为主线，形成统一的视觉与交互语言。通过强调色驱动的动态色阶、平台适配的玻璃常量、以及标准化的动画配置，实现了跨平台的一致体验。通用组件与业务组件解耦清晰，便于扩展与维护。
