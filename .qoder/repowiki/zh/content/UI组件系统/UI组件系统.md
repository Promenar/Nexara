...
- [src/features/chat/components/WorkspaceSheet/index.tsx](file://src/features/chat/components/WorkspaceSheet/index.tsx)
- [src/features/chat/components/WorkspaceSheet/WorkspacePathIndicator.tsx](file://src/features/chat/components/WorkspaceSheet/WorkspacePathIndicator.tsx)
- [src/features/chat/components/WorkspaceSheet/WorkspaceTabBar.tsx](file://src/features/chat/components/WorkspaceSheet/WorkspaceTabBar.tsx)
- [src/features/chat/components/WorkspaceSheet/ArtifactList.tsx](file://src/features/chat/components/WorkspaceSheet/ArtifactList.tsx)
...
- [src/features/chat/components/WorkspaceSheet/TaskList.tsx](file://src/features/chat/components/WorkspaceSheet/TaskList.tsx)
- [src/lib/skills/definitions/workspace.ts](file://src/lib/skills/definitions/workspace.ts)
- [src/features/chat/components/input/hooks/useKeyboardTracking.ts](file://src/features/chat/components/input/hooks/useKeyboardTracking.ts)
- [app/_layout.tsx](file://app/_layout.tsx)
- [app/(tabs)/chat.tsx](file://app/(tabs)/chat.tsx)
- [src/components/rag/KGNodeEditModal.tsx](file://src/components/rag/KGNodeEditModal.tsx)
- [src/components/rag/KGEdgeEditModal.tsx](file://src/components/rag/KGEdgeEditModal.tsx)
...
## 更新摘要
**变更内容**
- 在KGNodeEditModal、KGEdgeEditModal、WorkspacePathIndicator和BackupSettings组件中添加了KeyboardAvoidingView组件，改善了模态表单的键盘交互体验
- 实现了统一的键盘避免策略：iOS使用padding行为，Android使用adjustResize机制
- 保留了聊天输入框的KeyboardStickyView特殊处理
- 移除了工作区面板相关UI组件，包括WorkspaceSheet及其子组件
- 撤销了聊天界面键盘处理改进的相关变更
- 删除了工作区路径指示器、标签栏、任务列表、工件列表和文件浏览器组件
- 移除了工作区技能定义和相关文件系统操作
...
- **新增**：键盘避免系统，统一处理iOS padding行为和Android adjustResize机制。
- **新增**：JIT动态建图功能的UI组件支持，包括RagOmniIndicator的JIT进度指示器和RagAdvancedSettings的配置面板。
- **移除**：工作区面板相关UI组件，包括WorkspaceSheet及其子组件。
- **撤销**：聊天界面键盘处理改进的相关变更。
...
- **新增**：键盘避免系统，统一处理模态表单的键盘交互
- **移除**：工作区面板相关组件（WorkspaceSheet及其子组件）
...
KSV["KeyboardStickyView"]
KPR["KeyboardProvider"]
end
subgraph "RAG设置组件"
...
- **新增**：键盘避免系统，统一处理模态表单的键盘交互，支持iOS padding行为和Android adjustResize机制。
...
- **新增**：键盘避免系统提供统一的键盘交互处理，确保模态表单在不同平台上的最佳体验。
- **新增**：RagOmniIndicator与RagAdvancedSettings组件集成JIT动态建图功能，提供完整的RAG检索流程可视化。
- **移除**：工作区面板相关组件不再集成到聊天界面中。

```mermaid
sequenceDiagram
participant App as "应用"
participant Provider as "ThemeProvider"
participant KeyboardProvider as "KeyboardProvider"
participant Store as "SettingsStore"
participant Utils as "ColorUtils"
participant UI as "UI组件"
participant KeyboardSystem as "键盘避免系统"
participant RAG as "RAG系统"
App->>Provider : 初始化
Provider->>KeyboardProvider : 包裹应用
KeyboardProvider->>Store : 读取强调色与主题偏好
KeyboardProvider->>Utils : 生成动态色阶
Utils-->>KeyboardProvider : 返回色阶对象
KeyboardProvider-->>UI : 提供主题上下文(颜色/深浅/主题模式)
UI->>KeyboardSystem : 请求键盘避免处理
KeyboardSystem-->>UI : 应用平台特定的键盘行为
RAG->>UI : 更新RAG状态与进度
UI-->>RAG : 显示JIT进度指示器
```
...
#### KeyboardAvoidingView 统一处理
- **新增**：在KGNodeEditModal、KGEdgeEditModal、WorkspacePathIndicator和BackupSettings组件中统一添加KeyboardAvoidingView组件
- **新增**：iOS平台使用padding行为，自动为键盘留出空间
- **新增**：Android平台使用adjustResize机制，整体UI向上移动
- **新增**：与全局KeyboardProvider配合，确保键盘事件正确传递

```mermaid
flowchart TD
Start(["模态表单显示"]) --> CheckPlatform{"检查平台"}
CheckPlatform --> |iOS| ApplyPadding["应用padding行为<br/>KeyboardAvoidingView behavior='padding'"]
CheckPlatform --> |Android| ApplyResize["应用adjustResize机制<br/>behavior=undefined"]
ApplyPadding --> MoveContent["内容自动上移<br/>为键盘留出空间"]
ApplyResize --> MoveUI["整体UI上移<br/>避免键盘遮挡"]
MoveContent --> End(["完成"])
MoveUI --> End
```

**图表来源**
- [src/components/rag/KGNodeEditModal.tsx:183-315](file://src/components/rag/KGNodeEditModal.tsx#L183-L315)
- [src/components/rag/KGEdgeEditModal.tsx:80-144](file://src/components/rag/KGEdgeEditModal.tsx#L80-L144)
- [src/features/chat/components/WorkspaceSheet/WorkspacePathIndicator.tsx:138-205](file://src/features/chat/components/WorkspaceSheet/WorkspacePathIndicator.tsx#L138-L205)
- [src/features/settings/BackupSettings.tsx:550-623](file://src/features/settings/BackupSettings.tsx#L550-L623)
...
KAV["KeyboardAvoidingView"] --> TP
KAV --> ST
KAV --> CU
KSV["KeyboardStickyView"] --> TP
KPR["KeyboardProvider"] --> TP
IDX["ui/index.ts"] --> BTN
IDX --> GST
IDX --> LTH
IDX --> TST
...
- **新增**：键盘避免系统优化：统一处理iOS padding行为和Android adjustResize机制，减少平台差异带来的性能损耗。
- **新增**：JIT缓存机制：支持JIT结果缓存，提高重复查询性能，减少LLM调用次数。
- **新增**：密度检查优化：仅在现有图谱覆盖不足时触发JIT，避免不必要的处理开销。
- **移除**：工作区面板相关组件减少了不必要的视图层次和内存占用。
...
- **新增**：键盘避免问题
  - 检查KeyboardAvoidingView组件是否正确导入Platform模块。
  - 确认iOS平台使用padding行为，Android平台使用adjustResize机制。
  - 验证KeyboardProvider是否正确包裹应用根组件。
- **新增**：JIT功能异常
  - 检查JIT配置是否正确启用，最大块数设置是否合理。
  - 确认LLM模型可用性和网络连接状态。
  - 查看JIT缓存是否正常工作，必要时清理过期缓存条目。
- **移除**：工作区面板相关问题
  - 如遇到工作区相关问题，确认相关组件已被正确移除。
  - 检查是否存在遗留的文件系统操作或状态管理。
...
## 结论
Nexara 的 UI 组件系统以"主题—颜色—动画—组件"为主线，形成统一的视觉与交互语言。通过强调色驱动的动态色阶、平台适配的玻璃常量、以及标准化的动画配置，实现了跨平台的一致体验。**新增的键盘避免系统**进一步提升了模态表单的键盘交互体验，通过统一的iOS padding行为和Android adjustResize机制，确保了在不同平台上的最佳用户体验。**新增的JIT动态建图功能**进一步增强了RAG系统的可视化和用户体验，通过RagOmniIndicator组件提供实时的处理进度反馈，通过RagAdvancedSettings组件提供完整的配置界面。**移除的工作区面板组件**简化了聊天界面的复杂度，提升了整体性能。通用组件与业务组件解耦清晰，便于扩展与维护。
...
### **新增**：键盘避免系统配置
- **KeyboardAvoidingView集成**：在KGNodeEditModal、KGEdgeEditModal、WorkspacePathIndicator和BackupSettings中统一添加
- **iOS平台**：使用padding行为，KeyboardAvoidingView自动为键盘留出空间
- **Android平台**：使用adjustResize机制，整体UI向上移动避免键盘遮挡
- **平台检测**：通过Platform.OS条件渲染不同的behavior属性
- **与KeyboardProvider协作**：确保键盘事件正确传递和处理
...
### **移除**：工作区面板相关组件
- **工作区路径指示器**：用于选择和切换工作区路径的组件
- **工作区标签栏**：包含任务、工件、文件三个标签页的导航组件
- **任务列表**：显示工作区内任务的列表组件
- **工件列表**：显示工作区内工件的列表组件
- **文件浏览器**：浏览工作区内文件的组件
- **工作区技能定义**：提供列出工作区、获取工作区状态等功能的技能定义

这些组件已被完全移除，相关的文件系统操作和状态管理也随之简化。

**章节来源**
- [src/features/chat/components/WorkspaceSheet/index.tsx:1-328](file://src/features/chat/components/WorkspaceSheet/index.tsx#L1-L328)
- [src/features/chat/components/WorkspaceSheet/WorkspacePathIndicator.tsx:1-292](file://src/features/chat/components/WorkspaceSheet/WorkspacePathIndicator.tsx#L1-L292)
- [src/features/chat/components/WorkspaceSheet/WorkspaceTabBar.tsx:1-80](file://src/features/chat/components/WorkspaceSheet/WorkspaceTabBar.tsx#L1-L80)
- [src/features/chat/components/WorkspaceSheet/ArtifactList.tsx](file://src/features/chat/components/WorkspaceSheet/ArtifactList.tsx)
- [src/features/chat/components/WorkspaceSheet/FileBrowser.tsx](file://src/features/chat/components/WorkspaceSheet/FileBrowser.tsx)
- [src/features/chat/components/WorkspaceSheet/TaskList.tsx](file://src/features/chat/components/WorkspaceSheet/TaskList.tsx)
- [src/lib/skills/definitions/workspace.ts:1-174](file://src/lib/skills/definitions/workspace.ts#L1-L174)

### **撤销**：聊天界面键盘处理改进
- **键盘跟踪钩子**：提供输入框焦点状态跟踪和动画样式的钩子函数
- **键盘提供者**：在应用根部提供键盘状态管理的上下文
- **聊天界面键盘处理**：在聊天界面中处理键盘显示和隐藏的逻辑

这些改进已被撤销，相关的键盘处理逻辑回归到更基础的状态管理方式。

**章节来源**
- [src/features/chat/components/input/hooks/useKeyboardTracking.ts:1-57](file://src/features/chat/components/input/hooks/useKeyboardTracking.ts#L1-L57)
- [app/_layout.tsx:1-191](file://app/_layout.tsx#L1-L191)
- [app/(tabs)/chat.tsx:1-229](file://app/(tabs)/chat.tsx#L1-L229)