...
- [app/chat/[id]/settings.tsx](file://app/chat/[id]/settings.tsx)
- [app/chat/super_assistant/settings.tsx](file://app/chat/super_assistant/settings.tsx)
- [app/rag/editor.tsx](file://app/rag/editor.tsx)
- [app/settings/rag-config.tsx](file://app/settings/rag-config.tsx)
- [src/components/ui/FloatingTextEditorModal.tsx](file://src/components/ui/FloatingTextEditorModal.tsx)
- [src/components/ui/FloatingCodeEditorModal.tsx](file://src/components/ui/FloatingCodeEditorModal.tsx)
- [package.json](file://package.json)
</cite>

## 更新摘要
**变更内容**
- 键盘避免系统从平台特定实现统一为 react-native-keyboard-controller 统一实现
- 在应用根组件中引入 KeyboardProvider，提供统一的键盘事件处理
- 在多个页面和组件中使用 KeyboardAvoidingView 和 KeyboardStickyView
...
- **更新**：键盘避免系统，统一使用 react-native-keyboard-controller 实现跨平台兼容的键盘交互处理。
...
- **更新**：键盘避免系统，统一使用 react-native-keyboard-controller 实现跨平台兼容的键盘交互
...
KHC["react-native-keyboard-controller"]
end
subgraph "RAG设置组件"
...
- **更新**：键盘避免系统提供统一的键盘交互处理，通过 react-native-keyboard-controller 实现跨平台兼容性。
...
participant KeyboardController as "react-native-keyboard-controller"
...
KeyboardProvider->>KeyboardController : 注册键盘事件处理器
KeyboardController->>Store : 读取强调色与主题偏好
KeyboardController->>Utils : 生成动态色阶
Utils-->>KeyboardController : 返回色阶对象
KeyboardController-->>UI : 提供主题上下文(颜色/深浅/主题模式)
UI->>KeyboardSystem : 请求键盘避免处理
KeyboardSystem-->>UI : 应用统一的键盘行为
RAG->>UI : 更新RAG状态与进度
UI-->>RAG : 显示JIT进度指示器
```
...
#### react-native-keyboard-controller 统一实现
**更新**：键盘避免系统已从平台特定实现统一为 react-native-keyboard-controller 统一实现：

- **新增**：在应用根组件 app/_layout.tsx 中引入 KeyboardProvider，提供统一的键盘事件处理
- **新增**：在多个页面中使用 KeyboardAvoidingView 和 KeyboardStickyView 组件
- **新增**：支持跨平台统一的键盘交互处理，包括 padding 行为和 adjustResize 机制
- **新增**：与 react-native-reanimated 集成，提供流畅的键盘动画效果

**章节来源**
- [app/_layout.tsx:38](file://app/_layout.tsx#L38)
- [app/_layout.tsx:158](file://app/_layout.tsx#L158)
- [app/chat/[id].tsx:585](file://app/chat/[id].tsx#L585)
- [app/chat/[id].tsx:731](file://app/chat/[id].tsx#L731)
- [app/(tabs)/rag.tsx:1200](file://app/(tabs)/rag.tsx#L1200)
- [app/settings/rag-config.tsx:32](file://app/settings/rag-config.tsx#L32)
- [app/chat/[id]/settings.tsx:218](file://app/chat/[id]/settings.tsx#L218)
- [app/chat/super_assistant/settings.tsx:432](file://app/chat/super_assistant/settings.tsx#L432)
- [app/rag/editor.tsx:217](file://app/rag/editor.tsx#L217)
...
KHC["react-native-keyboard-controller"] --> KPR
IDX["ui/index.ts"] --> BTN
IDX --> GST
IDX --> LTH
IDX --> TST
...
- **更新**：键盘避免系统优化：统一使用 react-native-keyboard-controller 实现跨平台兼容性，提升键盘交互性能。
...
- **更新**：键盘避免问题
  - 检查 KeyboardProvider 是否正确包裹应用根组件。
  - 确认 react-native-keyboard-controller 版本兼容性。
  - 验证 KeyboardAvoidingView 和 KeyboardStickyView 的使用是否正确。
  - 检查是否有多个 KeyboardProvider 实例导致的冲突。
- **新增**：JIT功能异常
...
## 结论
Nexara 的 UI 组件系统以"主题—颜色—动画—组件"为主线，形成统一的视觉与交互语言。通过强调色驱动的动态色阶、平台适配的玻璃常量、以及标准化的动画配置，实现了跨平台的一致体验。**更新的键盘避免系统**通过 react-native-keyboard-controller 实现了统一的跨平台键盘交互处理，提供了更好的键盘事件管理和动画效果。**新增的JIT动态建图功能**进一步增强了RAG系统的可视化和用户体验，通过RagOmniIndicator组件提供实时的处理进度反馈，通过RagAdvancedSettings组件提供完整的配置界面。**移除的工作区面板组件**简化了聊天界面的复杂度，提升了整体性能。通用组件与业务组件解耦清晰，便于扩展与维护。
...
### 更新：键盘避免系统配置
**更新**：键盘避免系统已统一为 react-native-keyboard-controller 实现：

- **KeyboardProvider集成**：在应用根组件中引入 KeyboardProvider，提供统一的键盘事件处理
- **KeyboardAvoidingView使用**：在多个页面中使用 KeyboardAvoidingView 组件处理键盘遮挡
- **KeyboardStickyView使用**：在聊天界面中使用 KeyboardStickyView 保持输入框位置
- **跨平台兼容性**：统一处理iOS padding行为和Android adjustResize机制
- **与reanimated集成**：与 react-native-reanimated 集成，提供流畅的键盘动画效果
- **版本要求**：使用 react-native-keyboard-controller@1.18.5 版本
...
**章节来源**
- [src/features/chat/components/input/hooks/useKeyboardTracking.ts:1-56](file://src/features/chat/components/input/hooks/useKeyboardTracking.ts#L1-L56)
