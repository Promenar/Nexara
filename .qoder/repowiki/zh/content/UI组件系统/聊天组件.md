...
- **更新** 思维步骤现在统一在 ToolCallBlock 的时间线中显示，不再作为单独的中间内容卡片显示
...
- [ToolExecutionTimeline.tsx:1-750](file://src/components/skills/ToolExecutionTimeline.tsx#L1-L750)
...
- **更新** 思维内容处理：MessageContent 现在将思维步骤统一在 ToolCallBlock 的时间线中显示，不再作为单独的中间内容卡片显示
...
- **更新** 思维处理层：思维步骤统一在 ToolCallBlock 的 ToolExecutionTimeline 中显示，提供完整的执行时间线
...
- MessageContent：
  ...
  - **更新** 思维步骤处理：移除了中间思维步骤卡片的独立显示，思维步骤现在统一在 ToolCallBlock 的时间线中显示
...
## 结论
Nexara 聊天组件通过"容器/上下文 + 原子块"的架构实现了高内聚、低耦合的消息渲染体系。**更新** 思维内容处理的改进使得思维步骤统一在 ToolCallBlock 的时间线中显示，提供了更清晰、完整的执行过程展示。数学与图表渲染器采用 WebView 承载外部库，兼顾质量与性能；推理设置与预设提供了直观的参数编辑与快速切换能力。整体设计在保证可维护性的同时，兼顾了用户体验与性能优化。
...
- 类型与接口
  ...
  - 参考路径：[chat.ts 类型定义:135-167](file://src/types/chat.ts#L135-L167), [skills.ts 类型定义:63-73](file://src/types/skills.ts#L63-L73), [chat 类型共享接口:35-73](file://src/store/chat/types.ts#L35-L73)