...
## 更新摘要
**变更内容**
- 移除工作区持久化功能：移除sessions表中的workspace_path列定义和相关迁移逻辑
- 移除工件表中的workspace_path列：移除artifacts表中的workspace_path列定义
- 移除工作区相关UI组件：移除工作区路径指示器和相关界面元素
- 移除工作区技能定义：移除工作区列表和状态检查技能
...
## 简介
本文件面向Nexara数据库设计，围绕基于SQLite + FTS5 + 向量支持的架构展开，系统阐述表结构与索引策略、向量存储与相似度计算、数据模型关系、迁移与版本管理、查询优化与大数据量处理、以及备份恢复与完整性保障。**更新**：移除了工作区持久化功能，简化了数据库架构，专注于核心会话、消息和知识管理功能。
...
- 向量检索：原生加速与降级路径
- 关键词检索：FTS5优先、LIKE回退
- **移除**：工作区持久化：移除workspace_path列定义及相关功能
- 备份与恢复：选择性恢复与事务保障
...
- [src/lib/db/schema.ts:1-397](file://src/lib/db/schema.ts#L1-L397)
- [src/lib/db/migration.ts:1-417](file://src/lib/db/migration.ts#L1-L417)
- [src/lib/db/session-repository.ts:1-430](file://src/lib/db/session-repository.ts#L1-L430)
...
- [src/lib/db/schema.ts:3-397](file://src/lib/db/schema.ts#L3-L397)
- [src/lib/db/migration.ts:8-417](file://src/lib/db/migration.ts#L8-L417)
- [src/lib/db/session-repository.ts:14-152](file://src/lib/db/session-repository.ts#L14-L152)
...
## 架构总览
整体架构围绕SQLite为核心，结合FTS5提升关键词检索效率，向量表配合原生模块实现相似度计算加速，并通过触发器与索引保障数据一致性与查询性能。**更新**：移除了工作区持久化功能，简化了数据库架构，专注于核心会话和消息管理。
...
### 数据模型与表结构
核心实体与关系如下：
- 会话（sessions）：记录会话元信息与配置，支持JSON字段保存复杂配置。**更新**：移除了workspace_path列，简化了会话表结构。
...
- 工件（artifacts）：工作台集成产物。**更新**：移除了workspace_path列，简化了工件表结构。
...
**图表来源**
- [src/lib/db/schema.ts:5-397](file://src/lib/db/schema.ts#L5-L397)
...
**章节来源**
- [src/lib/db/session-repository.ts:110-151](file://src/lib/db/session-repository.ts#L110-L151)
...
- **移除**：工作区持久化：移除sessions和artifacts表中的workspace_path列定义，简化数据库结构。
...
### 工作区功能移除
**更新**：移除了完整的工作区持久化功能，包括：
- 会话表中的workspace_path列定义
- 工件表中的workspace_path列定义
- 工作区相关的迁移逻辑
- 工作区路径指示器UI组件
- 工作区列表和状态检查技能

这些变更简化了数据库架构，移除了不必要的复杂性，专注于核心会话和消息管理功能。

**章节来源**
- [src/lib/db/schema.ts:30](file://src/lib/db/schema.ts#L30)
- [src/lib/db/schema.ts:366](file://src/lib/db/schema.ts#L366)
- [src/lib/db/migration.ts:154-159](file://src/lib/db/migration.ts#L154-L159)
- [src/lib/db/migration.ts:385-390](file://src/lib/db/migration.ts#L385-L390)
- [src/lib/skills/definitions/workspace.ts:1-173](file://src/lib/skills/definitions/workspace.ts#L1-L173)
- [src/features/chat/components/WorkspaceSheet/WorkspacePathIndicator.tsx:1-116](file://src/features/chat/components/WorkspaceSheet/WorkspacePathIndicator.tsx#L1-L116)
...
- **移除**：工作区相关组件不再依赖数据库中的workspace_path列。
...
- **移除**：工作区持久化：移除了workspace_path列的存储和查询开销，简化了数据结构。
...
- **移除**：工作区相关问题：由于工作区功能已被移除，相关的workspace_path列错误不再适用。
...
## 结论
该数据库设计以SQLite为核心，结合FTS5与原生向量搜索，实现了从会话消息到知识库与知识图谱的全链路数据管理。**更新**：移除了工作区持久化功能，简化了数据库架构，专注于核心会话和消息管理。新增的知识图谱JIT缓存机制显著提升了动态提取的性能和可扩展性，通过源类型跟踪和作用域支持增强了数据的可追溯性和隔离性。通过完善的索引策略、迁移机制与自修复能力，兼顾了易用性与稳定性；通过WAL、游标分页与原生加速等手段，满足移动端大数据量场景的性能需求。配套的备份恢复与完整性检查进一步增强了系统的可靠性。
...
- **移除**：工作区功能相关配置和使用说明，包括workspace_path列的移除影响。
