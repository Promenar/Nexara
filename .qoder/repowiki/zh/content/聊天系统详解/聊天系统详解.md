...
**更新** 本次更新特别关注原生模块集成和聊天输入解耦的重大改进，包括Sanitizer、TextSplitter、TokenCounter三个原生模块的深度集成，以及通过原生模块优化的内容净化、文本分片和令牌计数性能。
...
## 架构总览
聊天系统采用"状态机 + 管理器 + 控制器 + 类型约束 + 原生模块"的分层设计：
...
**更新** 新增的原生模块集成系统进一步提升了系统的性能和稳定性，通过Sanitizer、TextSplitter、TokenCounter三个原生模块的深度集成，实现了内容净化、文本分片和令牌计数的高性能处理。同时，聊天输入解耦优化使得系统架构更加清晰，插件化的内容净化管道、智能的CJK文本分片器和统一的令牌计数接口为系统的可扩展性和维护性奠定了坚实基础。
