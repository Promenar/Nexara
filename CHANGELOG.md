...
### 会话任务进度卡与可销项任务胶囊 (2026-07-31)
- **任务卡移入消息流**：任务计划不再常驻挤占输入框内部空间，改为作为紧凑卡片跟随会话内容展示；卡片仅保留任务标题、当前步骤与 `已完成/总数` 数字，不再展示容易误导的状态说明或额外进度条。
- **输入栏第三任务胶囊**：模型与 Token 芯片后新增同规格任务胶囊；只有本轮确实在生成时显示脉冲“正在生成”，生成已停止但计划未销项时改为静态“待确认 N”，点击可返回任务卡。
- **人工收口**：任务卡提供“继续处理”和“标记已完成”；人工完成会一次性销项全部未完成叶子步骤，使任务卡与胶囊立即消失，已完成旧计划也不再阻塞下一份计划创建。
- **竞态保护与可用性**：生成过程中禁用并在 ViewModel 层拦截续办/销项操作；任务胶囊保留 48dp 可点击区但与相邻芯片统一可视高度，按钮主次和卡片密度经模拟器视觉复核。
- **验证**：`:app:compileDebugKotlin` 通过；`TaskPanelLogicTest`、`ChatViewModelTest`、`TaskRepositoryTest` 共 35 项相关测试在隔离干净副本中通过；Android 15 模拟器实测静态“待确认 1”、卡片 `1/2` 及手动完成后双入口同步消失。
