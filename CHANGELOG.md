...
- **集中式 FAB 状态指示器与错误横幅彻底收敛（RagStatusFab）**：
  - **消除顶部粗暴挤压横幅**：彻底移除原知识库顶部生硬侵入、挤压搜索框与操作列表的错误通知条，将交互转移至右下角悬浮 FAB 原型状态微件；
  - **动态环形进度与任务交替动效**：当后台执行文档向量化或知识图谱抽取时，FAB 外延展示环形旋转进度圈，中心呈现任务类型图标；两类任务并发时图标以 1.8 秒周期平滑轮播；
  - **状态语义与自退场**：错误状态呈现红叉图标，成功状态呈现绿勾图标并在 3.5 秒后平滑隐退；
  - **向左展开卡片操作台**：轻触 FAB 平滑向左展开气泡卡片，呈现任务标题、线性微型进度条与详细原因，并集成“去设置”（直达记忆设置配置嵌入模型）、“重试”与“关闭”操作，再次点击平滑收起。
- **知识图谱拓扑图标纠偏（Icons.Rounded.Hub）**：
  - 将操作栏“知识图谱”入口按钮及 FAB 内部图谱抽取图标统一修正为网状拓扑节点图标（`Icons.Rounded.Hub`），消除原树状图标的语义不匹配。
- **会话工作区沙箱与知识库深度隔离**：
  - **禁止自动向量化**：加固 `FileEntryDao.getUnvectorizedSupportedFiles` 与 `ResourceExplorerViewModel`，从数据库查询与调度链路彻底隔绝非系统会话的工作区文件，杜绝私有沙箱文件被静默加入向量化队列；
  - **UI 标签干净呈现**：会话工作区文件在前端列表隐藏 `[已索引]`、`[索引中]` 等 RAG 标签及菜单项，仅在用户显式“转存到知识库”后才调度索引。
- **同层下钻式无缩进目录切换（RagFolderBreadcrumbsBar）**：
  - **废除窄屏树状缩进与首屏内联展开**：在下钻式文件管理模式中，彻底阻断任何目录（包括根目录下的「会话工作区」及普通自建文件夹）的默认内联展开行为（`allowTreeExpansion = false`）。首屏与各级子目录均只展示当前层级的直属一级子项，深度全部为 0，彻底消除首屏的树状嵌套视觉结构；
  - **点击完整切换下沉进入目录**：点击「会话工作区」、会话子目录或自建文件夹时，直接完整同层切换进入目标目录，呈现纯粹的标准平铺文件管理体验；
  - **全链路路径面包屑与系统返回联动**：顶部配备清晰的层级面包屑路径（如 `全部文件 › 会话工作区 › Say OK`）与 `←` 返回按钮，支持点击任意上级跳转，并联动 `BackHandler` 支持 Android 系统返回键逐层平滑回退。

### 知识库“全局资源管理器”架构升级与会话工作区统一挂载（2026-09-26）

- **全局资源管理器核心挂载仓库（CompositeGlobalWorkspaceRepository）**：
  - **无缝虚拟挂载**：在保留现有虚拟文件系统（VFS）物理隔离与数据库 Schema 零破坏的前提下，实现统一复合工作区仓库，将全局知识库根目录与所有活跃会话的工作区在根路径进行聚合挂载；
  - **系统级写保护安全边界**：虚拟根目录（`会话工作区`）与各会话目录严格禁止非法删除、物理清空、误移和外部污染，保障会话与底层数据绝对安全；
  - **会话文件一键转存至知识库**：会话工作区内的文件支持在文件树长按菜单中一键“转存到知识库”（流式安全拷贝到全局工作区），并自动触发后台分块与向量化调度。
- **文件树（FilesPanel & FileRow）视觉与交互全面升级**：
  - **专属视觉徽章与图标**：虚拟系统根目录呈现专属文件夹图标并配备“系统”胶囊徽章；各会话工作区子目录配备专属对话气泡图标并配备“会话”胶囊徽章，视觉层级清晰、精致美观；
  - **图标交互优化**：目录图标支持点击就地直接展开/折叠，长按菜单根据节点类型智能裁剪非法操作（如虚拟节点的删除/移动屏蔽）。
...
### 助手设置收敛、悬浮菜单统一、默认字体调整与会话智能重命名（2026-09-24）

- **助手设置页面（AgentEditScreen）视觉收敛与模型配置**：
  - **视觉规格收敛**：根治页面字体过大、分辨率突降的视觉割裂感，全页迁移至标准 `NexaraSettingsPageLayout`；Hero 角色舞台等比例收敛（头像 88dp -> 72dp，微光 116dp -> 96dp，相机浮标 32dp -> 26dp，角色名从 24sp `headlineSmall` 降为 16sp `titleMedium`，描述降为 12sp `bodySmall`，表单标签 11sp、输入字 14sp，调色板和图标网格 22dp）；
  - **默认模型配置落地**：新增“模型配置”标准设置分区与配置项，支持为助手直接绑定默认模型，点击无缝调起 `ModelPicker` 并在卡片上实时呈现当前选定模型。
- **全站悬浮菜单（附件与气泡长按菜单）视觉规范统一**：
  - 发送输入框附件浮动菜单（`AttachmentActionMenu`）与消息气泡长按上下文菜单（`MessageContextMenu`）对齐统一视觉规范；
  - 统一采用 16dp 大圆角、`surfaceContainer` 底色、1dp 微描边与柔和投影，搭配 20dp 规范 Leading Icon 与 `labelLarge` 文字，消除气泡与全站视觉不一致。
- **会话默认字体大小优化**：
  - 将会话设置中文字体默认大小从 14 像素调整为 15 像素（`SessionOptions.fontSize = 15`），平衡阅读舒适度与屏幕信息密度。
- **错误气泡垂直居中修复**：
  - 修复单行错误提示文字在红粉色圆角背景框中未能垂直居中的排版瑕疵，使用 `Box(contentAlignment = Alignment.CenterStart)` 确保单行文本严格垂直居中。
- **新会话智能自动命名与顶栏点击重命名**：
  - **三级降级链路保障**：首轮对话完成后自动触发 LLM 会话标题命名，链路优先采用用户预设的“快速模型（summaryModelId）”，故障或未配置降级为当前会话模型，仍故障降级为首条用户消息前 10 字；
  - **黄金宽度测算与清洗**：严格测算顶栏可用宽度（约 200dp）与首页折叠卡片宽度（约 180dp），将标题上限锁定为 10 字（`MAX_TITLE_LENGTH = 10`），并正则清洗“标题：/主题：/Title:”等多余前缀与中英文标点；
  - **顶栏直接重命名**：会话顶栏标题增加视觉可交互反馈与 14dp 编辑小笔图标，轻触直接唤起重命名弹窗支持随时调整。
- **测试与实机验证闭环**：
  - 全量 2,791 项 JVM 单元测试 0 失败通过，覆盖标题生成与清洗截断测试；
  - 在 Pixel 7 AVD 模拟器上完整实机交互，各界面截图核验无误。


### Agent 编辑助手全屏视觉重构与 Hero 角色舞台升级（2026-09-24）

- **顶部高质感 Agent Hero 角色舞台打造**：
  - 顶部重构为居中角色舞台，引入柔和径向微光（`RadialGradient`）氛围底座与 88dp 大尺寸圆形头像；
  - 头像右下角搭载半透明深色圆形拍照浮动徽标（带主题色细边框），强化角色定制感；
  - 头像下方实时同屏呈现加粗昵称（`headlineSmall`）与简介，营造沉浸式角色威仪。
- **表单输入纯净卡片流式化（彻底消灭嵌套双边框套娃）**：
  - 彻底废除卡片内生硬嵌套 `OutlinedTextField` 的 Double-Border 粗糙做法；
  - 打造 `InCardInputField` 流式卡片表单项，支持小字次级标签（名称/描述）、大字无框文本输入（`BasicTextField`）与细腻半透明分割线，大幅提升高级质感。
- **色彩与图标工坊全面升级**：
  - **图标矩阵**：由紧绷方块升级为 16dp 饱满圆角与选中态主题色 15% 微光底座，附带丝滑折叠展开面板；
  - **调色盘圆点**：预设色升级为带平滑滑动的 `LazyRow`，彻底根除末尾颜色在小屏被截断的 Bug，选中圆点加入弹性微缩放与描边；
  - **自定义色相滑块**：底座升级为真实全光谱彩虹渐变轨道（`Brush.horizontalGradient`），配合右侧 44dp 大圆角实时色块预览徽章，色彩调配反馈直观惊艳。
- **系统人格 Prompt 卡片与危险操作收敛**：
  - 系统提示词由占地巨大的生硬输入框重构为交互式配置卡片：左侧心理人格图标徽章，右侧紧凑状态指示胶囊（已配置绿色呼吸点 / 未设置灰色点）与向右导航箭头；
  - 知识库与检索配置项全面收敛进标准 `NexaraSettingsSection` 与 `NexaraSettingsItem`；
  - 删除助手重构为 16dp 大圆角轻量半透明红色背景，消除原有的粗暴双层红框。
- **严格契约门禁与双重验证**：
  - 严守 `ThemeSurfaceContractTest` 静态颜色 token 规范与 `BettboxGroupedSurfacesContractTest` 分组卡片契约，全量 2,790 项 JVM 单元测试 0 失败通过；
  - 在运行中的 Pixel 7 AVD 模拟器上实机交互并截屏核验，角色舞台、流式表单、彩虹滑块与提示词卡片均完美呈现。
...
- **技术泄露与冗余信息净化**：
  - 彻底移除记忆界面中生硬且粗暴估算的 Token 统计（`~X tokens`），消除底层技术泄漏；
  - 移除三重冗余的“记忆”大标题与顶部“总计: X”，消灭信息堆砌。
- **空态对齐文档页精品设计**：
  - 抛弃原本笨重生硬的深色大卡片方框，升级为与文档页完全同构的居中设计：72dp 全圆高亮底座（`CircleShape`）+ 脑部图标 + “暂无记忆”主副文案；
  - 搜索无结果时同步采用居中全圆底座设计（`memory-search-empty`）。
- **列表底部淡色统计 Footer**：
  - 有记忆数据时，在列表底部以极淡居中小字呈现“共 X 条记忆”，完全不干扰首屏纯粹视觉。
...
- **首页（对话 Hub）**：
  - 移除顶部低频全局搜索栏与孤立图标，将“新建 Agent”操作重构为右下角 Material 3 悬浮操作按钮（FAB，16dp 圆角，带 primaryContainer 主色，内嵌 88dp 底部避让间距），首屏纯粹清爽。
- **知识库主界面**：
  - 移除 Scaffold 顶部冗余的“知识库”大标题（与底栏导航解耦），以 `RagPortalTabSwitcher` 全宽胶囊分段滑动选择器作为首屏视觉锚点。
  - 提取常驻统一搜索栏，自适应文档与记忆检索文案（“搜索文档…”/“搜索记忆碎片与事实…”），记忆 Tab 同步支持即时检索与空态反馈。
  - 移除顶栏孤立的“知识图谱”按钮，将其移至文档操作行，与“上传文档”、“新建文件夹”达成单行三等高按钮并列排布。
- **契约与国际化**：
  - 同步更新 `AgentSessionsMaterialContractTest` 契约测试；
  - 补齐记忆搜索中英文字符串资源。

### 知识库与知识图谱视觉打磨与层级收敛（2026-09-24）

- **知识库主界面**：
  - 移除 TopAppBar 右上角冗余的记忆设置齿轮按钮（全局记忆与 RAG 设置统一由底栏"设置"页面的"知识与检索"专属模块承接，消除同屏出现两个齿轮图标的功能冗余与视觉杂乱）。
  - TopAppBar 规范化加入“知识库”主标题，右侧仅保留知识图谱入口，形成明确视觉定位。
  - **文档 / 记忆 Tab 升级**：弃用呆板且带打勾 `✓` 标记的 Material 3 默认 SegmentedButton，重构为现代全宽胶囊分段滑动选择器 `RagPortalTabSwitcher`（高雅微圆角背景容器、平滑过渡的纯色选中浮动卡片、集成文档/记忆图标与自适应文本），兼顾规范触控与视觉质感。
  - **统一垂直韵律**：彻底修复原 Tab 区域双重 16dp 留白（合计 32dp）导致的视觉断层，将 Tab、搜索栏、上传/新建按钮行之间的垂直间距统一收敛为 8–12dp，界面节奏整齐利落。
- **知识图谱页面**：
  - **重构模式选择器**：将原先臃肿近 50dp、缺乏垂直居中的方形色块按钮重构为标准 Material 3 `FilterChip`（全局、文档、概念），规范高度（32–34dp）与圆角，文本垂直居中，选中与未选中态质感细腻。
  - **对齐刷新按钮**：右侧刷新按钮由裸露细字的 TextButton 升级为 34dp 等高的 `FilledTonalButton`，集成刷新图标与文本，与左侧 Chips 达成水平中线与高度基线完美对齐。
  - **文档选择与无障碍入口提升**：文档模式下的文档选择器由 TextButton 升级为带文件图标、选中标题和下拉指示箭头的圆角卡片选择器；无障碍图谱列表入口统一为圆角 tonal 按钮。
- **契约与测试同步**：
  - 更新 `RagHomeScreenContractTest` 与 `RagReleaseAccessibilityTest` 确保顶栏干净无冗余设置齿轮；
  - 修复 `ReleaseConfigurationTest` 对当前 0.2.2-beta 版本元数据的同步断言。
...
- **行内 LaTeX 严格边界与流式缓存失效**：行内数学公式正则收敛为排除前后空格与转义符的严格断言，避免误伤货币符号；增加流式结束时的分段缓存重计算机制，杜绝单 `$` 跨块穿透导致的公式缺失。
- **会话工具解析开放**：在 `DefaultSessionToolResolver` 遗留空选策略下放行 `ToolRisk.EXTERNAL_WRITE`，恢复 `image_generation` 等外部写型内置工具的调用解析。
- **图表与代码块排版优化**：ECharts 隐藏右上角默认 Toolbox 图标，消除对坐标轴与图例文本的遮挡；深色模式代码高亮标点符号对比度提升至 `#CBD5E1`；代码块行号采用等宽字体与单行固定高度，消除外层组件内边距导致的行号与代码行基线错位。
- **无障碍与触控尺寸对齐**：Mermaid 工具栏按钮、调色盘色块与代码块操作区均确保 >= 48dp 规范触控区域；用户气泡多模态图片设定 120dp–240dp 弹性高度与裁切约束，避免图片被压扁变形。
- **会话删除容灾**：当会话物理工作区根目录丢失或不存在时，平滑降级为纯数据库清理模式并正确释放生成协调器，杜绝僵尸会话。
- **设置与交互细节打磨**：设置页底部补充留白，消除悬浮 Dock 遮挡；模型卡片未知类型本地化为“通用”；RAG 摘要清洗行首 Markdown 标题 `#` 标记；文档编辑器视口左右对称内边距；模型归属 Badge 横向展开；Provider 连接探测捕获结构化状态码与友好原因。


### 独立模型目录（2026-09-22）

- 四个公开数据入口归一化为规范模型和供应商条目，保留来源与作用域；GitHub Actions 定时构建并签名发布到 Pages。
- App 在恢复完成后检查目录，模型管理提供手动更新；固定公钥验签、哈希与版本验证、原子缓存及内置快照提供更新失败与离线回退。
- 模型列表保留供应商显示名、额度和能力；精确匹配不改写请求 ID，已识别网关前缀仅作为查找依据，用户编辑与模型状态保留。
- 供应商元数据与输入额度纳入备份往返；协议列表按流读取并传播取消，兼容 Claude 能力对象与 Gemini 模型字段。
- 索引启动恢复在缺失文件扫描完成后才启动处理器，失败后保留恢复门禁，避免扫描与完成清理交错导致重复入队。

### 审计修复与双库恢复（2026-09-22）

- 工作区创建事务以文件 FD 的随机扩展属性、文件标识及内容摘要验证归属；目录使用归属标记并拒绝清除新增内容。移动采用原子禁止覆盖，目录创建使用目录 FD。
- UTF-8 嗅探处理采样边界和短读，导入时校验完整文本；索引调度失败成功回滚后退还批次预算。DOCX 预检按实际解压字节限制资源使用。
- 接通离线 HTML 导入与文本提取，保留段落、列表和表格结构；统一分块保护 Unicode 字符边界，PDF 提取按文本预算及时停止。
- 搜索查询保留技术符号、否定、实体和比较语义；取消向上传播，不触发后续生成或工具执行。
- 模型目录测试分离固定解析夹具与真实资产不变量，避免固定记录数或上游兼容型号缺席造成更新误报。
- OpenAI 兼容流处理合法可空字段；错误事件、错误非空类型、缺失结束标记和工具终态冲突均不得生成成功状态。指定本机网关测试仅显式启用并发送有界合成数据。
- 双库恢复具备来源归档、认证阶段记录、关联映射、工作区及引用资产发布、旧附件入口和两侧知识库选择；未完成索引和外部执行需用户显式重试。公开 v17+v2 与内部 v17+v5 均有设备行为验证；公开 APK 的真实 schema 作为无用户数据夹具纳管。
- 编辑器在编辑、预览与分屏切换时保留共同面板的组合身份，避免非末尾选区归零及重复初始化；实际横屏与 IME 交互回归通过。性能夹具使用 Compose v2 测试规则消除错误线程上的重组。
- 本机 Android 模拟器执行例外记入 PDEC；[当前验证记录](docs/release/v0.2.1-beta-validation.md) 分别记录最终候选门禁、网关及人工验收边界和历史结果。

### Bettbox 开源 Material 3 视觉基线迁移（2026-08-26）

- **分组面贯通全部设置层级**：新增唯一 `BettboxListGroup` filled-card 组件，以 20dp 圆角和 `surfaceContainerLow` 承载一级设置分组、Provider/模型管理、备份、本地模型、技能、用量、Agent 配置及 RAG/检索深层页面；最终移除共享外描边，层级只由填色与留白表达，避免边框被误读为设置项边界。
- **记忆设置与首页列表减法**：记忆设置不再把整页控件塞进一个大框，也不再用无法对应触控面的横线切割；预设、检索参数、摘要模板和后续入口改为四个边界明确的语义分组。Agent 首页移除行间横线以及列表上下 8dp 的异色空带，保留连续行、圆角裁切与完整按压反馈，不再出现顶部/底部伪阴影。
- **首页业务列表与身份头部**：设置首页账户身份块移除“账户”分区标签，保留更突出的 56dp 头像与用户名；Agent、会话与 Provider 模型按各自业务对象维持统一触控节奏，避免惰性列表的填满约束制造整屏空白卡壳。
...
- **设置与管理列表**：设置分组改为 20dp 圆角 `surfaceContainer` 连续填充面，标准 `ListItem` 恢复 Material 3 主副文字、24dp 前导图标和稳定触控节奏；Provider 与模型管理列表同步使用同一填充表面与首尾圆角，不再散落为贴背景的自定义文本行。
...
- **当前验证与正式 APK**：设置首页、Agent/会话列表、Provider 表单与模型列表、备份及 RAG/检索深层页的受影响截图均已逐张检查；全量 JVM 2571 项（0 failure/error、14 skip）、100/100 Screenshot、AndroidTest 编译与 Lint（0 Error/Fatal）通过。当前源码经 clean 正式签名 R8 Release 构建，APK 18,909,055 bytes，SHA-256 `79d6b7c71b903fe783e4bd4d1625b6a3794154cfb292731c0efbd6f9759f8c5e`，包身份、唯一 signer、登记证书、敏感内容与本地推理制品排除、R8 输出、16 KiB zipalign 和 checksum 均通过；本轮无连接设备，物理真机上的按压态仍由用户继续验收。
...
- **会话删除兼容**：从未创建工作区的普通/历史会话使用独立 Room 原子删除事务，显式清理消息、附件、工具账本、任务、向量、知识图谱和审计关联，不再误入物理工作区身份校验而导致左滑删除失败。
- **根页面与列表节奏**：对话、知识库、设置三个根页面移除左侧标题，同时保留搜索、添加、图谱和配置动作；底部水滴导航结构、尺寸与视觉不变。会话列表采用 17sp/22sp 半粗标题、14sp/19sp 正文、6dp 层级间距与 80dp 最小行高，Provider、模型摘要和设置入口同步减少标签墙与无规则纵向留白。
- **模型与媒体能力**：模型编辑器允许修改并持久化请求时发送给上游的精确模型 ID，适配无法列出模型的 OpenAI-compatible Provider；类型与能力改为连续单选/开关列表。用户图片和工具生成图片均可打开全屏查看器，支持缩放、旋转、保存与分享；现有图像生成工具链继续复用同一结果预览路径。
- **实机诊断与动效**：发行版新增 512 KiB 有界 JSONL 诊断事件文件，只允许状态、计数与标识符，正文、Prompt、请求/响应、凭据和 URL 字段强制省略；设置页可直接导出。根标签淡入淡出、会话增删/重排和既有流式尾部缓动共同减少硬切，并继续遵循系统动画缩放。
- **验证与候选 APK**：全量 JVM 2560 项（0 failure/error、14 skip）、101/101 Screenshot、AndroidTest 编译与 Lint（0 Error/Fatal）通过；当前源码经 clean 正式签名 R8 构建，并通过包身份、唯一 signer、登记证书、敏感内容、本地推理制品排除、R8 四类输出、16 KiB zipalign 与 checksum 验真。APK 为 18,683,280 bytes，SHA-256 `a0dedb874a4d3bead8097ef8d405fc11a536a2ccca842def64701ad5acdb7a73`；当前无连接设备，未把历史安装结果冒充当前 APK 的真机证据。
...
- **头像导入与裁剪闭环**：个人头像和 Agent 自定义头像统一使用系统图片选择器与 1:1 裁剪流程，支持移动、缩放和旋转，输出限制为最长 512px；裁剪页使用与应用一致的 Material 3 明暗色、避让系统状态栏，并提供明确可点击的“完成”动作。头像文件改为内容哈希命名和原子发布，连续上传不同图片会产生新缓存身份，不再被图片缓存永久锁在首次上传结果；Agent 头像显示统一裁满圆形容器，避免非方图留出背景边。
- **Agent 头像持久化与首页身份列表**：Agent 编辑页的延迟保存不再依赖无人订阅的派生状态；自定义头像先成功提交 Room 再更新页面并清理裁剪临时文件，保存失败时保持可重试。Agent 首页直接消费持久化头像路径，使用连续 Material 3 `ListItem`、88dp 行高、56dp 圆形头像及 17sp/15sp 主副文字层级；没有新增卡片容器，底部三按钮导航保持零改动。
- **发行与数据继承门禁**：应用候选升级为 versionCode 3 / `0.2.1-beta`，按公开 `v0.1-beta` 实测 `nexara.db` v17 结构新增到 `nexara_v2.db` v18 的同签名覆盖升级链。接管会校验精确 Room identity 与关键列、完整 checkpoint WAL、保留旧库回退源并原子发布；Android FTS4 完整性检查在一次性可写私有副本上执行，避免只读 `quick_check` 误报且保持歧义源库零写入。迁移保全 Agent、Session、Message、旧附件 JSON、工作区/RAG 根、向量与任务子图，并在启动阶段为精确旧会话路径补建根身份；`/data/data` 与 `/data/user/0` 的系统 bind-mount 别名按文件身份识别，不放宽符号链接和越界路径防护。门禁写入并回读数据库、偏好、WAL-only 与物理文件哨兵，恢复注入文件的 SELinux MCS 标签后只允许 `adb install -r`，禁止卸载或清数据回退。最终 API 35 实测已完成公开 `v0.1-beta` 到当前正式签名 APK 的全部哨兵回读。对于曾走过 `v0.1→v0.2` 而同时遗留 v17+v5 的双库安装，本版在任何合并写入前 fail-close 并保留两库，不再静默选库；无损双库恢复仍作为显式阻断记录于 [v0.2.1-beta 验证账本](docs/release/v0.2.1-beta-validation.md)。
...
- **深层页面恢复标准 MD3 层级**：二、三、四级页面不再使用 10–14sp 的压缩文字映射，页面标题、设置主标题、辅助说明与分组标题分别恢复为 22sp、16sp、14sp 与 14sp 的 Material 3 角色；单行/双行设置项使用 56dp/72dp 节奏，保留至少 48dp 触控目标。提供商列表移除装饰性品牌图标前导列，模型列表收紧为统一 16dp 文字轴，表单、搜索、开关、单选与操作按钮继续保留真实功能。
- **一级页标准列表层级**：继续实机对照后确认廉价感主要来自前导图标列与压缩字号，而不是副标题本身。一级入口现使用无前导/无尾随装饰的 Material 3 双行 `ListItem`，主标题 16sp/24sp、副标题 14sp/20sp；副标题只解释功能，不展示动态状态。账户区独立为 88dp 身份头部，使用 56dp 头像与 22sp/28sp 用户名，不再与普通设置行共用层级；深层页面同步使用同一标准排版体系。
- **导航硬边界**：底部聊天、知识库、设置三按钮导航组件保持零改动；新增集成截图只用于验证设置页在既有导航容器内的整体构图与内容避让。
- **视觉与回归**：更新受影响截图并新增设置首页普通手机和底栏集成基线；将 SE 源图与无图标双行首页按同宽裁切并排复核，同时检查 2.0x 字体无裁切或重叠；全量 JVM 2140 项、101/101 Screenshot、Lint 0 Error/Fatal、AndroidTest 编译与 Debug APK 构建通过。深层页面标准 MD3 Token 收敛后重新 clean 构建稳定证书签名 R8 APK，包身份、版本、单一 signer、历史正式证书、敏感内容/本地推理制品排除、R8 四类输出、16 KiB zipalign 与 checksum 均通过；APK 18,369,076 bytes，SHA-256 `4b19f50fa7c383d69b4acc28eebc4965f484194fe022f8a45f67fe9587f6bbe1`。
...
- **当前签名候选**：从当前治理提交 `3535735f` clean 重建稳定证书签名 R8 APK；18,367,648-byte 产物 SHA-256 为 `d226395f2da82c943698e5e78a0ee363ddef5839389543382ce55d90ad732146`，包名/版本、唯一 signer、登记证书、敏感内容、GGUF/本地推理制品、R8 四类输出、checksum 与 16 KiB zipalign 均通过。
- **冷安装证据**：此前从同一生产源码生成的稳定证书签名 APK 在可见 API 35/36 模拟器均完成卸载旧包、冷安装、设备 `base.apk` 回拉同哈希、冷启动、前台存活和 crash/ANR 检查；tag workflow 将对最终远端重建产物重新执行同类门禁。
- **发行边界**：2026-07-29 已授权以 prerelease 形式放行；物理真机 TalkBack、完整焦点遍历与更广泛 OEM/IME 体验保留为 beta 后续验证项。
...
- **跨平台 Screenshot 确定性**：Task 14 首轮远端 CI 在 Ubuntu UTC 上准确暴露 9 张含时间文本的 Screenshot 与东八区 reference 相差 8 小时；截图验证与更新任务现在都从 JVM 启动阶段固定 `Asia/Shanghai`。第二轮远端 CI 继续暴露 3 张流式正文捕获到 140ms 尾部淡入不同中间帧；Preview/Screenshot 检查模式现在固定渲染动画终态，真实应用仍保留尾部淡入。本机以外部 `TZ=UTC` 取得两轮逐张相同的定点渲染哈希并完成 96/96 GREEN，仅在人工核对差异范围后更新 3 张流式 reference。
- **当前提交远端矩阵**：提交 `c192ccea` 的 Android CI run `29831794553` 最终全绿；quality、API 31、API 35 首轮通过。API 36 首轮被托管模拟器 `Process system isn't responding` 系统 ANR 对话框遮住通知权限按钮，失败截图、UI tree 与空应用 crash buffer 均已保留；仅重跑失败 job 后同一测试链 13 分 43 秒通过，最终 run conclusion 为 `success`。
...
- **水滴挤压切换**：选中指示器以弹簧位移在三个目的地间移动，过渡中横向拉伸、纵向收缩后回弹；快速反向切换从当前形变连续接续，不重置闪跳，系统关闭动画时直接到达终态。
- **胶囊水波纹边界**：保留 Material 原生 bounded ripple，但三个 Tab 的交互层统一使用 `CircleShape` 裁剪；按压时水波纹只在对应胶囊内扩散，不再出现方形槽位边框。
- **可访问性与图层安全**：三个目的地均保留 `Role.Tab`、选中语义、完整名称和至少 48dp 触控目标；指示器使用不透明主题预混色并覆盖图标层，避免移动首帧出现旧图标穿透。
- **视觉与设备门禁**：深色、浅色、英文 2.0x 和紧凑横屏四张手机 actual 已逐张检查；API 35 验证快速连点、形变中点、反向连续性和 0x 动画缩放，平板 Rail 不受影响。
- **阶段边界**：本轮是 Task 4 的实机观感返修，不改变后续搜索、模型选择器和设置树迁移范围；整体发行继续保持 NO-GO。

### Material 3 总收敛：锚定式附件动作簇（2026-07-20）

- **输入栏原位展开**：附件 `+` 使用真实布局坐标展开单一 Material 3 动作面板，并旋转为关闭语义；图片与完整上下文文档继续复用既有选择入口，不使用脱离锚点的 `DropdownMenu` 或 Popup 偏移猜测。
- **覆盖与状态安全**：透明关闭层位于整个聊天 `Scaffold` 之上，外部、顶栏和系统 Back 首次点击只关闭菜单，不穿透到底层动作；生成或文档导入开始后菜单立即关闭并禁用。布局坐标尚未回报时使用根容器内受约束的首帧位置，随后切换到实际锚点。
- **无障碍与视觉门禁**：动作行保持至少 48dp，展开/收起语义、RTL、2.0x、320dp 短窗口、真实 IME 和 0x 动画缩放均通过设备测试；普通、英/中 2.0x、紧凑视口和 RTL 五张 actual 已逐张检查。
- **阶段边界**：本轮只完成聊天附件动作簇；业务搜索、会话列表、模型选择器和设置树继续按后续任务迁移，整体发行保持 NO-GO。
...
- **路由后硬预算门禁**：最终系统提示、活动历史、完整文档、图片、工具与输出预留按 UTF-8 字节保守估算；稳定模型 ID 读取用户容量覆盖，最终远端 ID 解析目录。未知或超限在 Provider 网络请求前阻止，不静默截断、摘要或降级检索。
- **可靠重试与草稿时序**：重试期间旧 AI 回复保留在持久层但从新 Prompt 排除，只有新回复成功后才替换；Busy、路由、预算、流式、取消或持久化失败均不丢旧回复。文本、图片与文档统一在 Prompt 被接受后清空，失败后可直接修正并重发。
- **导出与消息分支**：Markdown/TXT 导出包含角色、时间、实际模型及附件元数据/全文，可重新上传作为完整上下文；稳定消息可创建独立会话，重映射消息父引用，拒绝不完整工具链并重置摘要、运行态、向量派生状态和工作区。
- **当前门禁**：全量 JVM 2021 项（0 failure/error、14 skip）、64/64 Screenshot、Lint 0 Error/Fatal、AndroidTest 编译与 API 31/35/36 相关 Compose 测试通过；Terra/Sol 双复审均清零 Critical/Important。签名 R8 APK 为 18,250,632 bytes，SHA-256 `50ccd9a2906dff4a7b54f6304be253bdbe1ffc6414df70fb33002acfc9558286`，API 35/36 完成同哈希冷安装、字节回读、启动与应用级 crash/ANR 复验。真实 Provider 六项运行时变量本会话未注入，因此本轮未把历史 Provider PASS 冒充当前复验；整体发行继续 NO-GO。
...
- **真机首轮反馈闭环**：首次引导首条回复完成后只持久化完成状态，不再重建导航图并跳回主界面；返回键仍以主界面为栈基座。知识库、全局记忆设置和检索设置统一使用受信任的应用私有工作区，旧版不存在的绝对路径会回退到默认工作区，初始化异常转为可见提示而非闪退。
- **聊天与模型管理布局收口**：聊天输入区的模型/Token 胶囊采用 48dp 触控目标和 34dp 可见高度；模型管理批量操作固定 2×2 间距，类型/能力标签使用紧凑可见面与独立 48dp 触控层，上下文字段固定 112×48dp 并使用数字键盘。9 张受影响基线经人工对照后更新，36/36 Screenshot validation 通过。
- **模型元数据解析纠偏**：内置规格匹配按精确字符串、精确正则、具体部分匹配、通用回退排序，不再因通用规则靠前吞掉具体模型；补齐 DeepSeek V4 Flash/Pro、MiniMax M3 与 MiniMax M2.7/highspeed 的上下文和能力信息。既有模型只在仍完全符合旧自动生成指纹时迁移，用户自定义名称或上下文不会被覆盖。
- **最终本地签名候选验真**：当前 `d8d31090` 完成 1915 项 JVM 测试、56/56 截图、Lint、Debug/deviceTest 构建、DocEditor API 31/35/36 各 31/31 及 API 36 完整 bulk/cold-start 门禁。稳定证书签名 R8 APK 为 18,173,215 bytes，SHA-256 `9b4f109c203b5ce8b428e43d48e8fbe537c3f7907a788a67480e41dc4b1f546b`；API 35/36 均完成 fail-closed 验证、冷安装、设备 APK 字节回读、冷启动和 crash/ANR 观察。当前分支远端矩阵与用户真机人工验收仍待完成。
...
- **RAG 交互闭环**：文件移动由占位动作改为真实持久化操作，部分失败保留失败项；普通文档与引用文档均可基于稳定目标重试索引；失败提示提供真实重试/关闭动作并补齐动态状态语义。
...
### v0.2-beta P0 安全与数据交付门禁收口 (2026-07-12)
- **密钥与备份安全**：API Key 默认脱敏显示，完整显示需要显式操作且在失焦/生命周期切换时自动回遮；备份可选择是否包含完整 Key，恢复采用鉴权、预检和事务写入，错误密码或损坏包不落库。
- **数据完整性**：Room schema、备份顺序、复合外键和进程重建恢复链路完成一致性加固；真实 Android 多阶段恢复验证覆盖进程被杀后的继续处理与无重复回放。
- **设备 UI 门禁**：补齐安全设置和备份真实界面的 Compose 测试，P0 最终通过 JVM、Lint、APK 构建和 Android 设备测试。
- **主要提交**：`637ca38`、`91c720e`、`ce047cc`、`b574251`、`9d8b72c`。

### v0.2-beta P1 Provider、工作区、KG、工具与分享主链闭环 (2026-07-12)
- **请求级 Provider 路由**：模型使用稳定复合 ID 解析到所属 Provider、协议、Endpoint 和密钥配置；禁用、删除、空 Key、错配和绕过路径在建连前显式失败。
- **Session 工作区隔离**：文件 UUID、父节点、路径、版本和工具执行上下文受当前 `workspaceRootUuid` 约束，跨 Session 访问和路径逃逸被拒绝。
- **真实 KG 回流**：微图节点/边映射为稳定路径并随聊天消息持久化，重新加载后仍可展示；无真实图谱时不生成假数据。
- **工具执行幂等**：新增持久化八状态 ledger、审批 ID 透传、原子 claim 和确定性结果消息；重复流片段、重复批准、并发执行及 Activity 重建不会重复触发副作用。
- **分享导入闭环**：`ACTION_SEND` / `ACTION_SEND_MULTIPLE` 经持久化 inbox、共享 importer 和索引队列逐项处理；支持崩溃修复、容量重试、去重、部分失败、同名稳定重命名和 Android 16 双分享。
- **资源管理器与事务索引**：资源管理器只观察当前 Session root，SAF 复用共享 importer；文件索引采用事务外候选构建、事务内哈希复核与向量/FTS/KG 原子切换，失败保留旧索引。
- **删除与进程恢复**：永久删除统一清理文件及全部派生数据；稳定 tombstone 支持进程死亡恢复，单个损坏记录不阻断其它工作区和向量队列。
- **真实 LLM 显式门禁**：新增仅由六项 `NEXARA_TEST_LLM_*` 环境变量启用的真实集成任务；默认单测始终跳过网络外呼，缺凭证明确 SKIP，Release Manifest 保持禁用明文 HTTP。
- **统一验证**：1232 个 JVM 测试零失败（14 跳过）、28 个 Android 设备测试零失败（2 个多阶段用例按设计跳过）、Lint 无 Error/Fatal、Debug APK 构建成功；备份 `prepare -> force-stop -> commit` 与恢复 `stage -> verify -> force-stop -> noReplay` 均通过。
- **主要提交**：`3f2a246`、`8a2cf6e`、`c4ef4f3`、`e288398`、`63346db`、`250d6c2`、`043b375`、`e0e97cb`、`51b3004`、`79a7da0`。
