...
CREATE TABLE `messages` (`id` TEXT NOT NULL, `session_id` TEXT NOT NULL, `role` TEXT NOT NULL, `content` TEXT NOT NULL, `model_id` TEXT, `status` TEXT, `reasoning` TEXT, `thought_signature` TEXT, `images` TEXT, `tokens` TEXT, `citations` TEXT, `rag_references` TEXT, `rag_progress` TEXT, `rag_metadata` TEXT, `rag_references_loading` INTEGER NOT NULL DEFAULT 0, `execution_steps` TEXT, `tool_calls` TEXT, `pending_approval_tool_ids` TEXT, `tool_call_id` TEXT, `name` TEXT, `planning_task` TEXT, `is_archived` INTEGER NOT NULL DEFAULT 0, `vectorization_status` TEXT, `layout_height` REAL, `tool_results` TEXT, `files` TEXT, `user_images` TEXT, `attachments` TEXT, `is_error` INTEGER NOT NULL DEFAULT 0, `error_message` TEXT, `created_at` INTEGER NOT NULL, PRIMARY KEY(`id`), FOREIGN KEY(`session_id`) REFERENCES `sessions`(`id`) ON UPDATE NO ACTION ON DELETE CASCADE );
...
CREATE TABLE `vectorization_tasks` (`id` TEXT NOT NULL, `type` TEXT NOT NULL, `status` TEXT NOT NULL, `doc_id` TEXT, `doc_title` TEXT, `session_id` TEXT, `user_content` TEXT, `ai_content` TEXT, `user_message_id` TEXT, `assistant_message_id` TEXT, `last_chunk_index` INTEGER NOT NULL, `total_chunks` INTEGER, `progress` REAL NOT NULL, `error` TEXT, `created_at` INTEGER NOT NULL, `updated_at` INTEGER NOT NULL, PRIMARY KEY(`id`), FOREIGN KEY(`session_id`) REFERENCES `sessions`(`id`) ON UPDATE NO ACTION ON DELETE CASCADE );
