...
`v0.2-beta` 当前处于发行候选收口阶段。后台持续生成等功能已经实现；完整设备/视觉矩阵、真实 API 四模型 smoke、分支 Android CI及本地稳定证书签名 R8 APK 的 API 35/36 冷安装均已通过。现存发行门禁是 GitHub 可验证的 tag 签名、签名候选的真机 TalkBack/人工业务验收及 GitHub Release。在这些证据全部关闭前，本仓库不会把开发分支产物描述为正式发行包。
